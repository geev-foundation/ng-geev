...
@Injectable()
export class GeevHttpConfiguration {
    ...
    handleNonGeevErrorResponse(response: HttpResponse<any>) {
        const self = this;
    ...
    handleGeevResponse(response: HttpResponse<any>, ajaxResponse: IAjaxResponse): HttpResponse<any> {
    ...
    getGeevAjaxResponseOrNull(response: HttpResponse<any>): IAjaxResponse | null {
    ...
    handleResponse(response: HttpResponse<any>): HttpResponse<any> {
        var ajaxResponse = this.getGeevAjaxResponseOrNull(response);
        if (ajaxResponse == null) {
            return response;
        }

        return this.handleGeevResponse(response, ajaxResponse);
    }
...
@Injectable()
export class GeevHttpInterceptor implements HttpInterceptor {
  
    protected configuration: GeevHttpConfiguration;
    private _tokenService: TokenService = new TokenService();
    private _utilsService: UtilsService = new UtilsService();
    private _logService: LogService = new LogService();

    constructor(configuration: GeevHttpConfiguration) {
  ...
  protected normalizeRequestHeaders(request: HttpRequest<any>):HttpRequest<any> {
    ...
    protected addAspNetCoreCultureHeader(headers:HttpHeaders):HttpHeaders {
        let cookieLangValue = this._utilsService.getCookieValue("Geev.Localization.CultureName");
    ...
    protected addTenantIdHeader(headers:HttpHeaders):HttpHeaders {
        let cookieTenantIdValue = this._utilsService.getCookieValue('Geev.TenantId');
        if (cookieTenantIdValue && headers && !headers.has('Geev.TenantId')) {
            headers = headers.set('Geev.TenantId', cookieTenantIdValue);
        }
    ...
    protected handleErrorResponse(error: any, interceptObservable: Subject<HttpEvent<any>>): Observable<any> {
        ...
        this.configuration.blobToText(error.error).subscribe(json => {
            ...
            var ajaxResponse = this.configuration.getGeevAjaxResponseOrNull(errorResponse);
            
            if (ajaxResponse != null) {
                this.configuration.handleGeevResponse(errorResponse, ajaxResponse);
            } else {
                this.configuration.handleNonGeevErrorResponse(errorResponse);
            }
